...
class TestPipeline(unittest.TestCase):
    ...
    def test_extract_from_csv(self, mock_extractor: MagicMock) -> None:
    ...
    def test_apply_custom_transform(self) -> None:
    ...
    def test_preview_no_data(self) -> None:
        """Test preview when no data is available"""
        result = self.pipeline.preview()
        self.assertIsNone(result)

    def test_preview_with_data(self) -> None:
...
class TestPipelineExecution(unittest.TestCase):
    ...
    def setUp(self) -> None:
        """Set up test fixtures"""
        self.sample_data = pd.DataFrame({"id": [1, 2, 3], "name": ["Alice", "Bob", "Charlie"], "age": [25, 30, 35]})

    @patch("snake_pipe.extract.CSVExtractor")
    @patch("snake_pipe.load.FileLoader")
    def test_simple_pipeline_execution(self, mock_file_loader: MagicMock, mock_csv_extractor: MagicMock) -> None:
        ...
        result = pipeline.extract_from_csv("input.csv").load_to_csv("output.csv").run()

        # Verify execution
        self.assertIsInstance(result, pd.DataFrame)
        self.assertEqual(len(result), 5)  # sample_data has 5 rows
        mock_extractor_instance.extract.assert_called_once()
        mock_loader_instance.load_csv.assert_called_once()

    def test_pipeline_execution_no_data(self) -> None:
    ...
    @patch("snake_pipe.extract.CSVExtractor")
    def test_custom_transform_execution(self, mock_csv_extractor: MagicMock) -> None:
        ...
        result = pipeline.extract_from_csv("input.csv").apply_custom_transform(add_full_name, "add_full_name").run()
