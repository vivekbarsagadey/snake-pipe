...
class ValidationResult:
    ...
    def add_error(self, message: str, column: Optional[str] = None, row_index: Optional[int] = None) -> None:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {"is_valid": self.is_valid, "error_count": len(self.errors), "warning_count": len(self.warnings), "errors": self.errors, "warnings": self.warnings, "stats": self.stats}
...
class DataValidator:
    ...
    def check_data_types(self, expected_types: Dict[str, str]) -> "DataValidator":
        ...
        for column, expected_type in expected_types.items():
            ...
            type_mapping = {"int": ["int64", "int32", "int16", "int8"], "float": ["float64", "float32"], "string": ["object", "string"], "datetime": ["datetime64[ns]", "datetime64"], "bool": ["bool"]}

            expected_dtypes = type_mapping.get(expected_type, [expected_type])

            if actual_type not in expected_dtypes:
                self.result.add_error(f"Column {column} has type {actual_type}, expected {expected_type}", column=column)

        return self

    def check_null_values(self, columns: Optional[List[str]] = None, max_null_percentage: float = 0.0) -> "DataValidator":
        """
        ...
        for column in check_columns:
            ...
            if null_percentage > max_null_percentage:
                self.result.add_error(f"Column {column} has {null_percentage:.2f}% null values " f"(max allowed: {max_null_percentage}%)", column=column)
    ...
    def check_unique_values(self, columns: List[str]) -> "DataValidator":
        ...
        for column in columns:
            ...
            if duplicate_count > 0:
                self.result.add_error(f"Column {column} has {duplicate_count} duplicate values", column=column)
    ...
    def check_value_ranges(self, range_constraints: Dict[str, Dict[str, Union[int, float]]]) -> "DataValidator":
        ...
        for column, constraints in range_constraints.items():
            ...
            if min_val is not None:
                ...
                if below_min > 0:
                    self.result.add_error(f"Column {column} has {below_min} values below minimum {min_val}", column=column)

            if max_val is not None:
                above_max = (self.df[column] > max_val).sum()
                if above_max > 0:
                    self.result.add_error(f"Column {column} has {above_max} values above maximum {max_val}", column=column)
    ...
    def check_allowed_values(self, value_constraints: Dict[str, List[Any]]) -> "DataValidator":
        ...
        for column, allowed_values in value_constraints.items():
            ...
            if invalid_values:
                self.result.add_error(f"Column {column} contains invalid values: {list(invalid_values)}", column=column)

        return self

    def apply_custom_validation(self, func: Callable[[pd.DataFrame], List[str]], description: str = "custom validation") -> "DataValidator":
        """
