...
class DataEnricher:
    ...
    def add_calculated_column(self, column_name: str, calculation: Callable[[pd.DataFrame], pd.Series],
                            description: str = "") -> 'DataEnricher':
        """
    ...
    def add_categorical_encoding(self, categorical_columns: List[str], 
                               encoding_type: str = 'label') -> 'DataEnricher':
        """
    ...
    def add_aggregated_features(self, group_by_columns: List[str], 
                              agg_column: str, agg_functions: List[str]) -> 'DataEnricher':
        """
        ...
        for func in agg_functions:
            try:
                ...
                self.df = self.df.merge(
                    agg_data.rename(columns={agg_column: new_column}),
                    on=group_by_columns,
                    how='left'
                )
    ...
    def add_text_features(self, text_column: str) -> 'DataEnricher':
        ...
        self.df[f'{text_column}_uppercase_ratio'] = (
    ...
    def add_binned_features(self, numeric_columns: List[str], bins: int = 5, 
                          labels: Optional[List[str]] = None) -> 'DataEnricher':
        """
