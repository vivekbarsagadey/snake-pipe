...
class WarehouseLoader:
    ...
    def __init__(self, warehouse_type: str, connection_params: Dict[str, Any]):
        """
    ...
    def _load_to_snowflake(self, df: pd.DataFrame, table_name: str, 
                          database: Optional[str], schema: Optional[str], 
                          if_exists: str, **kwargs):
    ...
    def _load_to_bigquery(self, df: pd.DataFrame, table_name: str,
                         dataset: Optional[str], schema: Optional[str],
                         if_exists: str, **kwargs):
    ...
    def _load_to_redshift(self, df: pd.DataFrame, table_name: str,
                         schema: Optional[str], if_exists: str, **kwargs):
        ...
        if params:
            ...
            for key, value in params.items():
                query_parameters.append(
                    self.client.ScalarQueryParameter(key, "STRING", str(value))
                )
    ...
    def get_table_info(self, table_name: str, database: Optional[str] = None,
        ...
        if self.warehouse_type != 'bigquery':
            ...
            return {
                ...
                'database': database,
                'schema': schema,
                'columns': result_df.to_dict('records') if result_df is not None else []
            }
